              ...
              <div className="text-xs text-muted-foreground space-y-1">
                <p>• 기획 10회 ({settings?.planningModel || 'gemini-3-flash-preview'}):
                  {settings?.planningModel === 'gemini-2.0-flash' ? ' 무료' :
                   settings?.planningModel === 'gemini-3-pro-preview' ? ' ~$3.00 🔥최신최고' :
                   settings?.planningModel === 'gemini-3-flash-preview' ? ' ~$0.15 🔥최신추천' :
                   settings?.planningModel === 'gemini-2.5-pro' ? ' ~$1.50' :
                   settings?.planningModel === 'gemini-2.5-flash' ? ' ~$0.20' :
                   settings?.planningModel === 'gemini-2.5-flash-lite' ? ' ~$0.10' : ' ~$0.10'}
                </p>
                <p>• 집필 50회 ({settings?.writingModel || 'gemini-2.0-flash'}):
                  {settings?.writingModel === 'gemini-2.0-flash' ? ' 무료' :
                   settings?.writingModel === 'gemini-3-pro-preview' ? ' ~$30.00 🔥최신최고' :
                   settings?.writingModel === 'gemini-3-flash-preview' ? ' ~$1.50 🔥최신' :
                   settings?.writingModel === 'gemini-2.5-pro' ? ' ~$17.50 ⭐안정적' :
                   settings?.writingModel === 'gemini-2.5-flash' ? ' ~$2.10' :
                   settings?.writingModel === 'gemini-2.5-flash-lite' ? ' ~$1.05' : ' ~$1.05'}
                </p>
              </div>
              {(settings?.writingModel === 'gemini-3-pro-preview' || settings?.writingModel === 'gemini-2.5-pro') && (
                <p className="text-xs text-green-600 mt-2">
                  💡 Pro 모델은 긴 컨텍스트를 잘 기억해서 스토리 일관성이 우수합니다!
                </p>
              )}
              {settings?.writingModel === 'gemini-3-pro-preview' && (
                <p className="text-xs text-blue-600 mt-1">
                  🔥 Gemini 3 Pro는 LMArena 1501 Elo로 최고 성능입니다!
                </p>
              )}
            </div>
          </CardContent>
